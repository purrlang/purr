...
let rec checkStmt (stmt: Ast.stmt) (ctx: context) : (context, Error.t) result =
  match stmt with
       ...
       | None ->
           ...
           (match checkExprType init resolved_ty ctx with
            ...
            | Ok () ->
                ...
                Ok { ctx with symbols = new_symbol :: ctx.symbols }))
           ...
           (match inferExprType end_ with
            ...
            | Some end_ty ->
                if start_ty <> end_ty then
                  ...
                  let ctx_with_var = { ctx with symbols = new_symbol :: ctx.symbols } in
...
let checkProgram program =
  ...
  List.iter (fun (bench: Ast.bench_def) ->
    if Hashtbl.mem bench_names bench.name then
      errors := Error.fromSpan bench.span
        (Printf.sprintf "Duplicate benchmark name: %s" bench.name) :: !errors
    else
      Hashtbl.add bench_names bench.name ()
  ) program.Ast.benches;
  ...
  (* M10.5: Type-check benchmark bodies *)
  List.iter (fun (bench: Ast.bench_def) ->
    (* Build function signature table for benchmarks *)
    let func_table = Hashtbl.create 16 in
    add_predeclared_functions func_table;
    ...
    (* Check run body *)
    (match checkStmtList bench.run_body init_ctx with
    | Error e -> errors := e :: !errors
    | Ok _ -> ())
  ) program.Ast.benches;
